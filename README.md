...
Static assets live in `icons/` (manifest icons, `icon128.png` doubles as the
store icon) and `store/` (`screenshot-1280x800.png`). Both are generated by
`store/gen_assets.py`, which ports `drawIcon()` from `background.js`; regenerate
after changing the icon geometry:
